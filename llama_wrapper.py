...
class LlamaWrapper:
    def __init__(
        ...
        if t.cuda.is_available() and default_device is not None:
    ...
    def generate_text_with_conversation_history(
        self, history: List[Tuple[str, Optional[str]]], max_new_tokens=50
    ) -> str:
        tokens = tokenize_llama(
            self.tokenizer,
            self.system_prompt,
    ...
    def get_logits_with_conversation_history(self, history: List[Tuple[str, Optional[str]]]):
        tokens = tokenize_llama(
            self.tokenizer,
            self.system_prompt,
            history,
