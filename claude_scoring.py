...
def make_gpt_request(human_input: str, sys_prompt: Optional[str] = None) -> str:

    if sys_prompt is None:
        sys_prompt = 'You are an unbiased, calibrated text evaluator. You follow all instructions exactly.'
    human_message = human_input.strip()
    ...
    for _ in range(20):
        try:
            response = client.chat.completions.create(
                ...
                messages=[{"role": "system", "content": sys_prompt}, {"role": "user", "content": human_message}],
                temperature=0.0,
                max_tokens=15,
                n=1,
            )
...
def make_prompt(question_answer: Tuple[str, str]):
...
def evaluate(question_answers: List[Tuple[str, str]]) -> List[Union[str, float]]:
    """
